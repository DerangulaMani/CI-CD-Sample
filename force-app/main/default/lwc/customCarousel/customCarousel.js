 ...
 */
 ...
 export default class CustomCarousel extends LightningElement {
     ...
     backSlide(){
         let slideIndex  = this.slideIndex - 1  // 1
         this.slideSelectionHandler(slideIndex)
     }
     forwardSlide(){
        let slideIndex  = this.slideIndex +1 // 3
        this.slideSelectionHandler(slideIndex) //3
     }
 
     slideSelectionHandler(id){
         if(id > this.slides.length){  2>3
             this.slideIndex = 1  
         } else if(id < 1){   //   2<1
             this.slideIndex = this.slides.length // 3
         } else {
             this.slideIndex = id  // 2  
         }
